...
const Items = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.data.filter((item: any) => {
      ...
      return item.itemName?.toLowerCase().includes(searchLower) || item.sellPriece?.toString().includes(searchLower)
    })
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-md border">
        <Table>
          <TableHeader className="bg-amber-100">
            <TableRow>
              <TableHead onClick={() => handleSort("itemName")} className="cursor-pointer">
                Item Name <ArrowUpDown className="ml-2 h-4 w-4 inline" />
              </TableHead>
              <TableHead onClick={() => handleSort("sellPriece")} className="cursor-pointer">
      ...
      <Popup isOpen={isPopupOpen} onClose={resetForm} title="Add Item" size="sm:max-w-md">
