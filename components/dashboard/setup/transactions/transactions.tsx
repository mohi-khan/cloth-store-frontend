...
const Transactions = () => {
  ...
  const [sortColumn, setSortColumn] = useState<keyof GetTransactionType>("transactionDate")
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc")
  const [searchTerm, setSearchTerm] = useState("")

  const [isPopupOpen, setIsPopupOpen] = useState(false)

  const [formData, setFormData] = useState<CreateTransactionType & { paymentType: "cash" | "bank" }>({
  ...
  const handleSelectChange = (name: string, value: string) => {
    if (name === "paymentType") {
      setFormData((prev) => ({
        ...
        isCash: value === "cash" ? true : false,
        bankId: value === "bank" ? prev.bankId : null,
      }))
  ...
  const sortedTransactions = useMemo(() => {
    return [...filteredTransactions].sort((a, b) => {
      ...
      if (typeof aValue === "string" && typeof bValue === "string" && !isNaN(Date.parse(aValue)) && !isNaN(Date.parse(bValue))) {
        return sortDirection === "asc" ? Date.parse(aValue) - Date.parse(bValue) : Date.parse(bValue) - Date.parse(aValue)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CreateTransactionType = {
        transactionType: formData.transactionType as "payment" | "recieved" | "contra",
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-md border">
        <Table>
          <TableHeader className="bg-amber-100">
            <TableRow>
              <TableHead onClick={() => handleSort("transactionType")} className="cursor-pointer">
                Type <ArrowUpDown className="ml-2 h-4 w-4 inline" />
              </TableHead>
              <TableHead onClick={() => handleSort("vendorName")} className="cursor-pointer">
              ...
              <TableHead onClick={() => handleSort("isCash")} className="cursor-pointer">
            ...
            {!transactions?.data ? (
              ...
              paginatedTransactions.map((transaction) => (
                <TableRow key={transaction.transactionId}>
                  ...
                  <TableCell>{transaction.isCash === true ? "Cash" : "Bank"}</TableCell>
                  <TableCell>{formatDate(transaction.transactionDate ? new Date(transaction.transactionDate) : null)}</TableCell>
                  <TableCell>
      ...
      <Popup isOpen={isPopupOpen} onClose={resetForm} title="Add Transaction" size="sm:max-w-2xl">
                  ...
                  value={
                    formData.vendorId
                      ? {
                          ...
                          name: vendors?.data?.find((v) => v.vendorId === formData.vendorId)?.name || "",
                        }
                      : null
                  }
                  onChange={(value) => handleSelectChange("vendorId", value ? String(value.id) : "0")}
                  ...
                  value={
                    formData.customerId
                      ? {
                          ...
                          name: customers?.data?.find((c) => c.customerId === formData.customerId)?.name || "",
                        }
                      : null
                  }
                  onChange={(value) => handleSelectChange("customerId", value ? String(value.id) : "0")}
                  ...
                  value={
                    formData.bankId
                      ? {
                          ...
                          name: bankAccounts?.data?.find((b) => b.bankAccountId === formData.bankId)?.bankName || "",
                        }
                      : null
                  }
                  onChange={(value) => handleSelectChange("bankId", value ? String(value.id) : "0")}
